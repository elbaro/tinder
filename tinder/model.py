...
class Model(object):
    ...
    def train(self,
        ...
        # for epoch in :
        for1 = tqdm.trange(self.bundle.epoch + 1, epochs + 1,
                           desc='epoch') if interactive else range(self.bundle.epoch + 1, epochs + 1)
        for epoch in for1:
            metrics = self.train_epoch(train_loader, train_minibatch_fn,
                                       logging, manual_opt, interactive=interactive)
